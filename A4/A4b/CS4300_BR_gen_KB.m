...
for i = 1:16 %sets up rule for breezes where one neighbor must be a pit
    ...
    for j = 1:16
        if conGraph(i,j) == 1
            KBi(length(KBi) + 1).clauses = sort([-(j + pit_offset), i + breeze_offset]);
    ...
    KBi(length(KBi) + 1).clauses = sort(new_rule);
...
for i = 1:16 %sets up rule for stenches where one neighbor must be a wumpus
    ...
    for j = 1:16
        if conGraph(i,j) == 1
            KBi(length(KBi) + 1).clauses = sort([-(j + wumpus_offset), i + stench_offset]);
